...
\end{block}
{\footnotesize Se integran los 4 estados $[x,\dot x,\theta,\dot\theta]$; el
control sólo usa $\theta$. $l$ = distancia pivote--centro de masa.}
\end{frame}

\begin{frame}{Discretización ZOH analítica $\rightarrow$ modelo ARX}
\begin{block}{Se discretiza una única vez; luego todo es discreto}
...
\]
\begin{itemize}
  \item Perturbación escalón $\Rightarrow$ $R$ con factor $1-q^{-1}$ (operador
  diferencia): al despejar $u$ aparece su inversa $\Rightarrow$ acción integral.
  \item Ruido de medición $\Rightarrow$ factor $z+1$ en $S$. El RLS \textbf{no
  filtra} el ruido (puede sesgar la estimación) $\Rightarrow$ mejoras de robustez.
\end{itemize}
\end{block}
\end{frame}
...
\begin{enumerate}
  \item \textbf{Acción integral}: diseñar sobre $\bar A = A(1-q^{-1})$ (grado 3),
  luego $R = (1-q^{-1})\bar R$.
...
\begin{frame}{Resultados: pulso rectangular ($t\in[6,8]$, en la salida)}
\begin{center}
...
rectangular (no un escalón sostenido).
\end{frame}

\begin{frame}{Resultados: ruido de medición ($\sigma$ sólo en $y_{\text{medida}}$)}
\begin{center}
...
\begin{itemize}
  ...
  amplifica: $b_1$ cambia de signo, $s_1$ crece $\sim4\times$ y deriva.
  \item Consecuencia: los polos de LC \emph{sobre el modelo nominal} se corren al
  borde ($0{,}989$--$0{,}995$), perdiendo margen. Con ruido incluso superan
  $|z|=1$ en algunos rediseños (diseño momentáneamente inestable).
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item Regresor construido con la salida medida (no resuelve errores en
  variables; sólo da consistencia).
...
\end{itemize}
\end{block}
Resultado: $R,S,T$ convergen y se recuperan los polos de diseño
($\max|z|=0{,}9418$). \emph{Ojo}: como el prior $\approx$ planta, esto demuestra
sobre todo \textbf{robustez} (ver slide de autoajuste).
\end{frame}

\begin{frame}{Comparación detallada: sinusoidal y ruido}
\begin{columns}
\begin{column}{0.5\textwidth}
\includegraphics[width=\textwidth]{imagenes_pid_autotunning/str_cmp_detalle_Pert_Sinusoidal.png}
\centering {\scriptsize Sinusoidal}
\end{column}
\begin{column}{0.5\textwidth}
\includegraphics[width=\textwidth]{imagenes_pid_autotunning/str_cmp_detalle_Ruido_Medicion.png}
\centering {\scriptsize Ruido}
\end{column}
\end{columns}
\centering \small Básico (trazo punteado): $S$ deriva y el radio de polos sube (o
supera $|z|=1$). Robusto (trazo sólido): coeficientes constantes, margen
preservado.
Con ruido, RMS$_\theta$ baja $\sim6\times$.
\end{frame}

\begin{frame}{Convergencia del regulador (resumen)}
\begin{center}
\includegraphics[width=0.72\textwidth]{imagenes_pid_autotunning/str_convergencia.png}
\end{center}
\centering \small std$(s_0)$ en la ventana final (log): básico (rojo) no converge
con sinusoidal/ruido; robusto (azul) converge siempre. Validado con
\texttt{test\_str\_pendulo.py} (todos los tests pasan).
\end{frame}

\begin{frame}{Autoajuste (1): carro 50\% más pesado $\Rightarrow$ cambia $b$}
\begin{columns}
\begin{column}{0.55\textwidth}
\includegraphics[width=\textwidth]{imagenes_pid_autotunning/str_self_tuning.png}
\end{column}
\begin{column}{0.45\textwidth}
{\small
Con prior $\approx$ planta, el robusto \emph{congela} el nominal (robustez). Con
planta distinta + excitación se ve el \textbf{self-tuning}:
\begin{itemize}
  \item ganancia estimada $\to$ real
  ($b_1{+}b_2: -5{,}9\!\to\!-3{,}9\times10^{-4}$);
  \item $S$ se rediseña: $[-367,697,-331]\to[-551,1048,-498]$; final $\neq$ nominal, estable.
\end{itemize}
\textbf{$a_1,a_2$ no cambian}: $a_2\equiv1$ (estructural) y $a_1$ depende de
$A_\theta\!\propto\!\frac{M+m}{4M+m}$, casi insensible a $M$. La masa sólo afecta
la ganancia $B_\theta\!\propto\!1/M$.}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{Autoajuste (2): barra 10\% más larga $\Rightarrow$ mueve $a_1$}
\begin{columns}
\begin{column}{0.55\textwidth}
\includegraphics[width=\textwidth]{imagenes_pid_autotunning/str_self_tuning_l.png}
\end{column}
\begin{column}{0.45\textwidth}
{\small
Alargar la barra sí cambia $A_\theta$ y por lo tanto el polo:
\begin{itemize}
  \item $a_1: -2{,}00632 \to -2{,}00599$ (real $-2{,}00575$); $b$ también converge;
  \item $S$ se rediseña a $[-402,766,-364]$ (planta real); lazo estable.
\end{itemize}
$a_1$ multiplica a $y\approx0$ (regulado) $\Rightarrow$ peor excitado que $b$
(multiplica a $u$): requiere \textbf{más dither} y \textbf{menor zona muerta}.}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
  \item STR indirecto \textbf{en tiempo discreto} (RLS + Diofantina/Sylvester +
  ley RST, con acción integral y sin cancelar el cero no de fase mínima) que
  estabiliza la dinámica angular del péndulo invertido.
  \item El diseño nominal reproduce exactamente los polos deseados; la variante
  robusta mantiene el desempeño acotado ante perturbaciones y ruido (RMS$_\theta$
  hasta $\sim6\times$ menor con ruido, margen preservado).
  \item Con prior $\approx$ planta y regulación, los 4 escenarios demuestran sobre
  todo \textbf{estabilidad y robustez}.
  \item El \textbf{autoajuste} propiamente dicho se validó aparte: con planta
  $50\%$ más pesada y excitación, el RLS va del prior a la planta real y el RST se
  rediseña a un controlador \emph{distinto} del nominal, estable.
  \item Clave práctica: numerador diminuto en el denominador del diseño
  $\Rightarrow$ proteger la estimación (excitación, zona muerta, gate).
\end{itemize}
\end{frame}
